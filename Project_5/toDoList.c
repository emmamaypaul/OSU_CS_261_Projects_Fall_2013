...
/* Name: Emma Paul
 * Date: Nov 24, 2013
 * Solution description: This program creates a to do list and sorts the list based in priority
 *
 */
...
Task* createTask (int priority, char *desc)
{
	...
	strcpy(newTask->description, desc);//pointer to the description string, don't quite understand how it works
	printf("createTask\t ");
	printf("New Task, Priority %d\t", newTask->priority);//task priority
...
void loadList(DynArr *heap, FILE *filePtr)
{
	...
	//char desc[TASK_DESC_SIZE], *nlptr;//how to fix this?
	//int priority;
...
{
    /* FIXME: Write this */
	assert(sizeDynArr(heap) > 0);//pre: the list is not empty
	DynArr * tempCopy = createDynArr(sizeDynArr(heap));//make a temp copy of the heap so heap isn't deleted
	copyDynArr(heap, tempCopy);
	//sortHeap(tempCopy); this screws up the code
	Task* data1;
	while(sizeDynArr(tempCopy) > 0)
			{
				...
				//free(data1); this screwed up the code
			}
	//freeDynArr(tempCopy); this screws up the code
...
int compare(void *left, void *right)
{
	...
	Task* firstTask;
	Task* secondTask;
	firstTask = (Task*) left;//cast
	secondTask = (Task*) right;

	//function to compare to task types
	if (firstTask->priority < secondTask->priority) return -1;
	if (firstTask->priority > secondTask->priority) return 1;
