...
/* CS261- Assignment 5 - dynamic array.c*/
/* Name: Emma Paul
 * Date: Nov 24, 2013
 * Solution description: This program implements common methods associated with a heap
 *
 */
...
{
  /* FIXME */
	assert(i < heap->size);//pre:	i < size
	assert(j < heap->size);//pre:   j < size
	if(compare(getDynArr(heap, i), getDynArr(heap, j)) == -1)//if i is less than j
	{
		return i;
	}
	return j;
...
{
  /* FIXME */
	assert(!isEmptyDynArr(heap));//pre: heap is not empty
	return getDynArr(heap, 0); //heap->data[0]; //returns value of first node
}
...
{
    /* FIXME */
	assert(heap->data != 0);//pre: heap is not null, not sure if done right

	int parent; //variable for parent node
	int pos = sizeDynArr(heap); // next open position is the size of the array, so for a size of five, 0-4 is filled, so 5 is the next open position
	addDynArr(heap, val); //sets capacity of necessary, puts value passed into pos

	while(pos !=0) //recursive percolating loop, until the position points at the root node //(index 0) and there are no nodes above it to percolate through
	{
		parent = (pos-1)/2; //calculates parent of position (for any index) (*floor of result)
			if(compare(getDynArr(heap, pos), getDynArr(heap, parent)) == -1)//gets values //of pos and parent and compares them, if position value is greater, it returns -1
			{
				swapDynArr(heap, parent, pos); //swaps parent and position
				pos = parent; //update position to the parent node to keep percolate up //on the next loop around
			}
			else return; //if position is less than parent then you just return
		}
	}
...
{
   /* FIXME */
	assert(max <= heap->size); //pre: max <= size
	int leftIdx = pos * 2 + 1; //computes left child
	int rightIdx = pos * 2 + 2; //computes right child
		...
		if(rightIdx < max)//if right is less than max we know we have 2 children
		{
			...
			if(compare(getDynArr(heap, smallerIndex), getDynArr(heap, pos)) == -1)//if necessary (pos is greater than smallest), swap values
				swapDynArr(heap, smallerIndex, pos);
			_adjustHeap(heap, max, smallerIndex);//recursive part- to go to next adjustment step
		}
		else if (leftIdx < max) //only has 1 child and we know its on the left
		{
			if(compare(getDynArr(heap, leftIdx), getDynArr(heap, pos)) == -1)//compare child (leftIdx) to parent (pos), if child is less than
...
void removeMinHeap(DynArr *heap)
{
   /* FIXME */
	assert(!isEmptyDynArr(heap)); //pre: heap is not empty
	int last = sizeDynArr(heap) -1; //tells us where last element is located
	putDynArr(heap, 0, getDynArr(heap, last));  //copy the last elements value (from getDynArr(heap, last) and put it as the first index position)
	removeAtDynArr(heap, last); //removes last element
	_adjustHeap(heap, last, 0); //recursive method that percolates down from index 0 up to //but not INCLUDING last …which is beyond the end now!
