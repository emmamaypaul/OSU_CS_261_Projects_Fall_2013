...
/* Name: Emma Paul
 * Date: November 11, 2013 (using late pass)
 * Solution description:This file contains typical functions for of a binary search tree
 * Note: I got help from my Ian Paul and referenced my groups worksheets to help me complete the assignment.
...
/*
 ...
 pre:	val is not null
 */
struct Node *_addNode(struct Node *cur, TYPE val)//referenced lecture
{
	if (cur == NULL) //base case , when current node is null and has no value
		{
			struct Node *newnode = malloc(sizeof(struct Node)); //create a new node
			assert (newnode != 0);// make sure allocation works
			newnode->val = val; //new node gets value passed in
			newnode->left = newnode->right = NULL; //set it’s left and right pointers to null
			return newnode; //returns the newnode
		}
		if (compare(val, cur->val) == -1)//recursive cases using compare, if value < current value
		{
			cur->left = _addNode(cur->left, val); //current essentially gets shifted to the left if value is less than current value
		}

		else //otherwise the value > current value
		{
			cur->right = _addNode(cur->right, val);//so we call recursive add node function to shift a node to the right
		}
			return cur; //returns current node for recursive loop to happen again
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	assert(tree != NULL);
	assert(val != NULL);
	struct Node *cur = tree->root;//current node points to tree root at first
	while (cur != NULL)//while the current node is pointing to a node
	{
		if (compare(val, cur->val) == 0)//if they are equal, found it!
		{
			return 1; //return true
		}
		else if(compare(val, cur->val) == -1)//if value is less than current node's value
		{
			cur = cur->left; //update current to the left node
		}
		else //if value is greater than current node
		{
			cur = cur->right; //update current to the node to the right
		}
	}
		return 0;//otherwise return false if not found
}
...
{
	assert(cur != NULL);
	  struct Node *temp = cur;//temp node equals current node passed in

	    while(temp->left != NULL)//while the temp node has a value
	    {
	    	temp = temp->left;//update it so the left
	    }

	    return temp->val;//returns left most value
...
{
	/*write this*/
	/*below is a way I tried to do from the hint that caused errors
	assert(cur != NULL);
	if(cur->left != NULL)//if its left child is NOT NULL
		{
			cur = cur->left;//returns that left null
		}
		else //otherwise it's the leftmost node
		{
			struct node *temp = cur; //temp node equals current
			cur = cur->right;//returns right child of cur
			free (temp); // and frees current
			//cur->left = _removeLeftMost(cur->left);
		}
		*/

	if(cur->left == NULL)//if there isn't a left node, special case
	{
		return cur->right;//return the right node
	}
	else //there is a left node
	{
		cur->left = _removeLeftMost(cur->left);//so we update the left after we remove the leftmost node
	}

	return cur; //and return the current node
}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	assert(cur != NULL);
	assert(val != NULL);
	//assert(containsBSTree(cur, val)); (causes an error)
	 if(compare(val, cur->val) == 0)//base case if you've found the node
	    {
	        if(cur->right == NULL)//if right child is NULL , special case
	            return cur->left;//return the left child

	        else
	        {
	            cur->val = _leftMost(cur->right);//replace value with leftmost child of right child
	            cur->right = _removeLeftMost(cur->right);//set right child to be the node after you have removed the left most node
	        }
	    }

	    else if(compare(val, cur->val) == -1)//recursive case, if value < current value
	    {
	        cur->left = _removeNode(cur->left, val);//updates current to the left
	    }
	    else //else value > current value
	    {
	        cur->right = _removeNode(cur->right, val);//so it updates current to the right
	    }

	    return cur;//returns current node
